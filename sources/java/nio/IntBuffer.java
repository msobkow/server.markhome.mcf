 ...
 * on a {@linkplain java.nio.Buffer#isReadOnly read-only} IntBuffer. The
 * methods {@linkplain #array array} and {@linkplain #arrayOffset arrayOffset}
 * throw an {@linkplain UnsupportedOperationException} if the IntBuffer is
 * not backed by an {@linkplain Buffer#hasArray accessible int array}
...
public abstract sealed class IntBuffer
    extends Buffer
    ...
    HeapIntBuffer, DirectIntBufferS, DirectIntBufferU,
...

{
    ...
    IntBuffer(int mark, int pos, int lim, int cap,   // package-private
                 int[] hb, int offset, MemorySegment segment)
    {
    ...
    //
    IntBuffer(int mark, int pos, int lim, int cap, MemorySegment segment) { // package-private
        this(mark, pos, lim, cap, null, 0, segment);
    }

    // Creates a new buffer with given base, address and capacity
    //
    IntBuffer(int[] hb, long addr, int cap, MemorySegment segment) { // package-private
    ...
    IntBuffer putArray(int index, int[] src, int offset, int length) {
    ...
    @Override
    int scaleShifts() {
        return Integer.numberOfTrailingZeros(Integer.BYTES);
    }
    ...
    AbstractMemorySegmentImpl heapSegment(Object base,
        ...
        return SegmentFactories.arrayOfIntSegment(base, offset, length, readOnly, bufferScope);
    }
