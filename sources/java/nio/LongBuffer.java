 ...
 * on a {@linkplain java.nio.Buffer#isReadOnly read-only} LongBuffer. The
 * methods {@linkplain #array array} and {@linkplain #arrayOffset arrayOffset}
 * throw an {@linkplain UnsupportedOperationException} if the LongBuffer is
 * not backed by an {@linkplain Buffer#hasArray accessible long array}
...
public abstract sealed class LongBuffer
    ...
    HeapLongBuffer, DirectLongBufferS, DirectLongBufferU,
...

{
    ...
    LongBuffer(int mark, int pos, int lim, int cap,   // package-private
                 long[] hb, int offset, MemorySegment segment)
    {
    ...
    //
    LongBuffer(int mark, int pos, int lim, int cap, MemorySegment segment) { // package-private
        this(mark, pos, lim, cap, null, 0, segment);
    }

    // Creates a new buffer with given base, address and capacity
    //
    LongBuffer(long[] hb, long addr, int cap, MemorySegment segment) { // package-private
    ...
    LongBuffer putArray(int index, long[] src, int offset, int length) {
    ...
    AbstractMemorySegmentImpl heapSegment(Object base,
        ...
        return SegmentFactories.arrayOfLongSegment(base, offset, length, readOnly, bufferScope);
    }
