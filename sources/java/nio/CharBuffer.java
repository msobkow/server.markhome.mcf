 ...
 * on a {@linkplain java.nio.Buffer#isReadOnly read-only} CharBuffer. The
 * methods {@linkplain #array array} and {@linkplain #arrayOffset arrayOffset}
 * throw an {@linkplain UnsupportedOperationException} if the CharBuffer is
 * not backed by an {@linkplain Buffer#hasArray accessible char array}
...
public abstract sealed class CharBuffer
    ...
    HeapCharBuffer, DirectCharBufferS, DirectCharBufferU,
...

{
    ...
    CharBuffer(int mark, int pos, int lim, int cap,   // package-private
                 char[] hb, int offset, MemorySegment segment)
    {
    ...
    //
    CharBuffer(int mark, int pos, int lim, int cap, MemorySegment segment) { // package-private
        this(mark, pos, lim, cap, null, 0, segment);
    }

    // Creates a new buffer with given base, address and capacity
    //
    CharBuffer(char[] hb, long addr, int cap, MemorySegment segment) { // package-private
    ...
    CharBuffer putArray(int index, char[] src, int offset, int length) {
    ...
    @Override
    int scaleShifts() {
        return Integer.numberOfTrailingZeros(Character.BYTES);
    }
    ...
    AbstractMemorySegmentImpl heapSegment(Object base,
        ...
        return SegmentFactories.arrayOfCharSegment(base, offset, length, readOnly, bufferScope);
    }
